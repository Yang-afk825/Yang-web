# -*- coding: utf-8 -*-
"""æºè½è§£ç å¼æ â èªå¨æ£æµç¼ç ç±»åå¹¶é¾å¼è§£ç .

æ¯æç¼ç ç±»å:
    base64, base32, base16(hex), base58, base62, base85, base91
    URLç¼ç , HTMLå®ä½, ROT13, äºè¿å¶, å«è¿å¶, Unicodeè½¬ä¹, æ©æ¯çµç 
"""
import re
import base64
import binascii
...
#  ç¼ç æ£æµå½æ° â è¿åç½®ä¿¡åº¦ 0-100
# âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
# âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
#  ç¼ç æè¿° & æ£æµå¨æ³¨åè¡¨
# âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

ENCODING_DETECTORS: List[Tuple[str, str, Callable[[str], int]]] = [
    ("binary",     "äºè¿å¶ 0101",          _is_binary),
    ("octal",      "å«è¿å¶ \\123",         _is_octal),
    ("decimal",    "åè¿å¶ ASCII ç ",       _is_decimal),
    ("morse",      "æ©æ¯çµç  .-",          _is_morse),
    ...
    ("url",        "URL ç¼ç  %xx",        _is_url_encoded),
    ("html",       "HTML å®ä½ &amp;",      _is_html_entity),
    ("unicode",    "Unicode è½¬ä¹ \\u",   _is_unicode_escape),
    ("rot13",      "ROT13",                _is_rot13),
]


def detect_encoding(text: str) -> List[Tuple[str, str, int]]:
    """æ£æµææ¬æå¯è½çç¼ç ç±»å. è¿å: [(ç¼ç ID, æè¿°, ç½®ä¿¡åº¦0-100), ...] æç½®ä¿¡åº¦éåºæå."""
...
def chain_decode(text: str, max_depth: int = 10) -> List[Tuple[str, str, str]]:
    """èªå¨æ£æµå¹¶é¾å¼è§£ç . è¿å: [(ç¼ç ID, ç¼ç æè¿°, è§£ç ç»æ), ...]."""
    chain = []
    current = text.strip()
    seen = {current}
...
def brute_decode(text: str) -> List[Tuple[str, str, str, str]]:
    """å°è¯ææè§£ç å¨, è¿åææè½äº§çå¯æå°ç»æçåæ¯."""
    ...
    for enc_id, (decoder, _) in DECODERS.items():
        try:
            ...
            if decoded and decoded != text.strip() and len(decoded) >= 2:
                readable = "â å¯è¯»" if is_printable(decoded) else "â ä¸å¯è¯»"
                results.append((enc_id, enc_desc_map.get(enc_id, enc_id), decoded, readable))
        except Exception:
            pass
    return sorted(results, key=lambda x: "â" in x[3], reverse=True)
